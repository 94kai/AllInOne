...
const state = { view: 'home', roots: [], root: '', path: '', entries: [], bookmarks: [], favorites: [], grid: false, lastSystemUpdate: 0 };
...
function setGreeting() {
  ...
  $('#page-title').textContent = state.view === 'home' ? greeting : state.view === 'files' ? '文件空间' : '服务入口';
...
async function loadSystem() {
  ...
  try {
    ...
    $('#cpu-value').textContent = `${data.cpu}%`; $('#cpu-meta').textContent = `${data.cores} 核 · 单次采样`;
    ...
    const availableDisks = data.disks.filter(disk => !disk.unavailable);
    const total = availableDisks.reduce((sum, disk) => sum + disk.total, 0); const used = availableDisks.reduce((sum, disk) => sum + disk.used, 0);
    const diskPercent = total ? used / total * 100 : 0;
    $('#disk-value').textContent = total ? `${diskPercent.toFixed(0)}% 已用` : '不可用'; $('#disk-meta').textContent = total ? `${formatBytes(used)} / ${formatBytes(total)}` : '请检查目录配置';
    $('#disk-progress').style.width = `${diskPercent}%`; $('#host-status').textContent = `${data.hostname} 在线`;
...
function renderFiles() {
  ...
  container.innerHTML = state.entries.map(entry => {
    ...
    return `<button class="file-row" data-file-path="${escapeHtml(entry.path)}"><span class="file-main"><span class="file-icon ${entry.type}"><svg viewBox="0 0 24 24">${icon}</svg></span><span class="file-name">${escapeHtml(entry.name)}</span></span><span class="file-size">${entry.type === 'folder' ? '文件夹' : formatBytes(entry.size)}</span><span class="file-date">${date}</span><span class="row-arrow"><svg viewBox="0 0 24 24"><path d="m9 18 6-6-6-6"/></svg></span></button>`;
  }).join('');
}
...
async function loadFiles(nextPath = state.path) {
  ...
  try {
    const data = await request(`/api/files?${new URLSearchParams({ root: state.root, path: nextPath })}`);
    state.path = data.path; state.entries = data.entries; renderFiles(); $('#file-hint').textContent = data.truncated ? '仅显示前 5000 项' : '安全只读模式';
...
async function openPreview(entry) {
  ...
  } else body.innerHTML = `<div class="preview-placeholder"><svg viewBox="0 0 24 24">${icons.file}</svg><p>暂不支持在线预览此格式</p><a class="primary-button" href="${fileUrl(entry, true)}">下载文件</a></div>`;
}
...
function renderLinks() {
  const card = (item, editable) => `<a class="link-card" href="${escapeHtml(item.url)}" target="_blank" rel="noopener"><span class="link-symbol" style="background:${item.color}">${escapeHtml(item.title.slice(0,1).toUpperCase())}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description || new URL(item.url).host)}</p>${editable ? `<button class="link-menu" data-edit-link="${escapeHtml(item.id)}" aria-label="编辑 ${escapeHtml(item.title)}"><svg viewBox="0 0 24 24">${icons.more}</svg></button>` : ''}</a>`;
...
function openLinkForm(item) {
  ...
  $('#link-description').value = item?.description || ''; $('#link-color').value = item?.color || '#5b8def'; $('#link-form-title').textContent = item ? '编辑地址' : '添加地址';
...
$('#refresh-button').addEventListener('click', () => state.view === 'home' ? loadSystem() : state.view === 'files' ? loadFiles() : loadLinks());
...
$('#link-form').addEventListener('submit', async event => {
  ...
  const body = { title: $('#link-title').value, url: $('#link-url').value, description: $('#link-description').value, color: $('#link-color').value };
  try {
...
document.addEventListener('keydown', event => { if (event.key === 'Escape') { if (!$('#preview-modal').hidden) closeModal('preview'); if (!$('#link-modal').hidden) closeModal('link'); } });
