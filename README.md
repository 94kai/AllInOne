...
主导航支持通过 `?view=` 直接打开并自动切换，例如把 `http://127.0.0.1:2006/?view=terminal` 保存为书签即可直达“终端”。可用值为 `home`、`files`、`speed`、`checklist`、`beijing-pass`、`terminal`、`xiaoai` 和 `music`。

## 进京证

进入“进京证”Tab 后点击“查询进京证”。结果按车辆展示车牌、办理状态、有效期、类型、申请时间、证号、驾驶人及剩余次数等信息，不直接显示原始 JSON。接口调用链默认折叠，展开后可查看状态查询和 JWT 换取两个接口、脱敏凭据、本次步骤状态和最终使用路径。服务先使用当前状态 Auth 调用状态接口；失败时使用 JWT 调用 `getSsoUserToken`，将响应 `data` 保存为新状态 Auth 后重试。页面“接口配置”只读展示两个固定接口地址，方便以后按地址抓包；状态 Auth 和 JWT 仍可编辑。真实凭据仅保存于 Git 忽略的 `data/beijing-pass/config.json`，文件权限由模块保存时收紧为 `0600`，不要将其提交或分享。
